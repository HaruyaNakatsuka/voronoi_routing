...
def plot_routes(
    ...
    instance_name="",
    output_dir="figures",
    elapsed_time=None,  # ★追加
):
    """
    各車両の経路を描画し保存する関数（等距離線付き）
    - elapsed_time: 実行時間（秒, float）
    """
    ...
    def load_step_routes(step_index):
        ...
        try:
            ...
            comp_costs, total = company_costs(
                j["routes"], id2, j.get("vehicle_num_list", vehicle_num_list)
            )
    ...
    if len(depot_id_list) > 1:
        ...
        x_vals = np.linspace(
            min(c["x"] for c in customers) - 10,
            max(c["x"] for c in customers) + 10,
            300,
        )
        y_vals = np.linspace(
            min(c["y"] for c in customers) - 10,
            max(c["y"] for c in customers) + 10,
            300,
        )
    ...
    if iteration == 0:
        ...
        lines.append("【ボロノイ分割後】" if iteration == 1 else "【GAT最適化実行後】")
        ...
        for i, c in enumerate(curr_company, 1):
            lines.append(
                f"  LSP {i}: {fmt(c)}   "
                f"改善(ラウンド比): {pct(prev_metrics['company'][i-1] if prev_metrics else None, c)}   "
                f"改善(ボロノイ比): {pct(voro_metrics['company'][i-1] if voro_metrics else None, c)}   "
                f"改善(初期比): {pct(init_metrics['company'][i-1] if init_metrics else None, c)}"
            )
        lines.append(
            f"  TOTAL: {fmt(curr_total)}   "
            f"改善(ラウンド比): {pct(prev_metrics['total'] if prev_metrics else None, curr_total)}   "
            f"改善(ボロノイ比): {pct(voro_metrics['total'] if voro_metrics else None, curr_total)}   "
            f"改善(初期比): {pct(init_metrics['total'] if init_metrics else None, curr_total)}"
        )

    # ====== ★ 実行時間をフッターに追加 ======
    if elapsed_time is not None:
        lines.append(f"\nGAT実行時間: {elapsed_time:.2f} 秒")
