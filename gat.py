...
def optimize_intra_company_by_ortools_2vehicle_gat(original_routes, customers, PD_pairs, vehicle_capacity):
    """
    ...
    for i in range(num_vehicles):
        for j in range(i + 1, num_vehicles):
            ...
            if new_cost < old_cost:
                ...
                # 探索空間拡張のために、"new routesのタスク全交換ルート"もアクション集合に追加する
...
def optimize_intra_company_by_exact_2vehicle_gat(original_routes, customers, PD_pairs, vehicle_capacity):
    """
    社内限定GAT：与えられた routes は単一会社ぶんのみを想定。
    - 2車両ペアごとに部分問題を解き、改善候補（アクション）を集める
    - 各車両は最大1回だけ変更されるようにCP-SATでアクションを選択
    ...
    for i in range(num_vehicles):
        for j in range(i + 1, num_vehicles):
            ...
            # 両車両の（開始=終了）デポ
            start_i = original_routes[i][0]
            start_j = original_routes[j][0]
            start_depots = [start_i, start_j]
            end_depots = [start_i, start_j]
            ...
            new_routes = solve_exact_2vehicle_vrp(
                ...
                sub_PD_pairs=sub_PD_pairs,     # ここは list[(p_id,d_id)]
                start_depots=start_depots,    # ここは [depot_id_for_v0, depot_id_for_v1]
                end_depots=end_depots,        # ここもID
                vehicle_capacity=vehicle_capacity,
            )
            if new_routes is None:
                continue
